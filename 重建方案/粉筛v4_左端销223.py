# -*- coding: utf-8 -*-
"""粉筛 v4：销在托架左端、销高 Y=223(主体中心高)、双耳对中 Z=0。
   水平连杆方案：连杆与销同高、近水平；摆杆连杆臂竖直向上。
   装料位销 X=-47, 左停 -95, 行程48。"""
import numpy as np
...
CY, CX_FILL, S = 223.0, -47.0, 48.0
O2s = np.array([-107.0, 33.0]); Rbs, RRs = 190.0, 10.0
LAM_MID = 90.0
...
def half(Ls):
    ...
    return brentq(g, 4, 20)

Ls = brentq(lambda L: Cx_of(LAM_MID-half(L), L) - CX_FILL, 25, 80)
hs = half(Ls); psi_s = 2*hs
lam_left, lam_fill = LAM_MID+hs, LAM_MID-hs

def mu0s_of(r0, Lrs):
    aO1 = np.degrees(np.arctan2(-O2s[1], -O2s[0]))
    a = np.linalg.norm(O2s)
    cg = (a*a + Lrs*Lrs - r0*r0)/(2*a*Lrs)
...
best = None
for Lrs in [50.0]:
    for r0 in np.arange(np.floor(abs(np.linalg.norm(O2s)-Lrs))+3, 135, 1.0):
        try:
            mu0 = mu0s_of(r0, Lrs)
        except ValueError:
            continue
        cd = cam_design(O2s, Lrs, mu0, psiS, RRs, -1)
        pu, re_ = amax(cd['alpha'],290,335), amax(cd['alpha'],90,135)
        if pu <= 28 and re_ <= 38:
            rm = cd['rpitch'].max()
            if best is None or rm < best[0]:
                best = (rm, Lrs, r0, mu0, cd, pu, re_)
            break
assert best is not None, "无解"
RMAX, LRS, R0S, MU0S, camS, PU, RE = best
...
print(f"摆角={psi_s:.3f}  连杆臂: 左停{lam_left:.2f} 装料{lam_fill:.2f}  滚子臂左停mu0={MU0S:.2f}  夹角={aS:.2f}")
print(f"凸轮v4: r0={R0S} 节曲线 {camS['rpitch'].min():.2f}~{camS['rpitch'].max():.2f} 实际 {camS['rprof'].min():.2f}~{camS['rprof'].max():.2f}")
print(f"压力角: 推卸 {PU:.2f}  左退 {RE:.2f}  最小曲率半径 {np.nanmin(np.abs(camS['rho'])):.1f}")
...
print(f"连杆最低点 Y={minBy:.2f} (台面216, 需>217)  传动角min {gB:.1f}")
print(f"B最左 X={min(Bf[0],Bl[0]):.1f}  摆杆竖臂X={O2s[0]:.0f} (台面左缘-80, 需臂X<-80)")
print(f"滚子臂段到主轴最小距离: {min(seg_min_r(O2s,Pf), seg_min_r(O2s,Pl)):.1f} (下冲头凸轮最大半径64)")

import os
OUT = os.path.dirname(os.path.abspath(__file__))
with open(os.path.join(OUT, "粉筛凸轮v4_SW.txt"), "w") as f:
    for x, y in camS['prof']:
        f.write(f"{x:.4f}\t{y:.4f}\t0\n")
    f.write(f"{camS['prof'][0,0]:.4f}\t{camS['prof'][0,1]:.4f}\t0\n")
np.savetxt(os.path.join(OUT, "粉筛凸轮v4.csv"),
...
print("已导出 粉筛凸轮v4_SW.txt / 粉筛凸轮v4.csv")
